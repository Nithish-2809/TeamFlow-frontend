...
function BoardTopBar({
  ...
  // Store
  const { sendInvite, leaveBoard, deleteBoard, pendingMembers, renameBoard } =
    useBoardPageStore();
  ...
  console.log("registering board:renamed listener")

  const handleBoardRenamed = ({ boardId: renamedBoardId, newName }) => {
    console.log("board:renamed received", renamedBoardId, newName)
    if (renamedBoardId === boardId) {
      renameBoard(newName)
    }
  }

  socket.on("board:renamed", handleBoardRenamed)
  return () => socket.off("board:renamed", handleBoardRenamed)
}, [boardId, renameBoard])
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        ...
        !pendingPanelRef.current.contains(event.target)
      ) {
  ...
  const handleLeave = async () => {
    ...
    try {
      ...
      setToast({ message: "You have left the board", type: "success" });
      setTimeout(() => navigate("/"), 1000);
    } catch (error) {
      setToast({
        ...
        <div className="board-topbar-right">
          ...
          <button className="topbar-btn" onClick={handleToggleMembers}>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path
                d="M11 14V12.6667C11 11.9594 10.719 11.2811 10.219 10.781C9.71897 10.281 9.04058 10 8.33333 10H3.66667C2.95942 10 2.28103 10.281 1.78103 10.781C1.28103 11.2811 1 11.9594 1 12.6667V14"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
              ...
              />
              <path
                d="M11 5.5C11.5933 5.5 12.1734 5.67595 12.6667 6.00559C13.1601 6.33524 13.5446 6.80377 13.7716 7.35195C13.9987 7.90013 14.0581 8.50333 13.9424 9.08527C13.8266 9.66721 13.5409 10.2018 13.1213 10.6213C12.7018 11.0409 12.1672 11.3266 11.5853 11.4424C11.0033 11.5581 10.4001 11.4987 9.85195 11.2716C9.30377 11.0446 8.83524 10.6601 8.50559 10.1667C8.17595 9.67339 8 9.09334 8 8.5"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
              />
          ...
          <button className="topbar-btn" onClick={handleInvite}>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path
                d="M11 14V12.6667C11 11.9594 10.719 11.2811 10.219 10.781C9.71897 10.281 9.04058 10 8.33333 10H4.66667C3.95942 10 3.28103 10.281 2.78103 10.781C2.28103 11.2811 2 11.9594 2 12.6667V14"
                stroke="currentColor"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
          ...
          {isAdmin && (
            ...
            >
              <button
                className="topbar-btn topbar-btn--pending"
                onClick={() => setShowPendingPanel((v) => !v)}
                title="Join Requests"
              >
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path
                    d="M8 1a5 5 0 0 0-5 5v2.5L1.5 10.5h13L13 8.5V6a5 5 0 0 0-5-5Z"
                    stroke="currentColor"
                    strokeWidth="1.5"
                    strokeLinejoin="round"
                  />
          ...
          <div className="topbar-menu-wrapper" ref={menuRef}>
            <button className="topbar-icon-btn" onClick={handleMenuToggle}>
            ...
            {showMenu && (
              <div className="topbar-dropdown">
                ...
                */}
                <button
                  className="dropdown-item dropdown-item--collapsed"
                ...
                {isAdmin && (
                  <button
                    className="dropdown-item dropdown-item--collapsed"
                    onClick={handleOpenPendingFromMenu}
                  >
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path
                        d="M8 1a5 5 0 0 0-5 5v2.5L1.5 10.5h13L13 8.5V6a5 5 0 0 0-5-5Z"
                    ...
                    {pendingMembers.length > 0 && (
                      <span className="dropdown-item__badge">
                        {pendingMembers.length}
                      </span>
                    )}
                  </button>
